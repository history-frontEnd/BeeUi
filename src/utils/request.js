...
const requestWithLogin = async (method, url, params = {}, showToast = false, handler = {}) => {
  let isLogin = false
  ...
  return new Promise((resolve, reject) => {
    handler.success = res => {
      ...
      if (res.statusCode === 200) {
        if (showToast && res.data && res.data.result === 'success') toast(res.data && res.data.message || '成功')
        Debug('response success: ' + url + ' ==============================')
        Debug(res.data)
        Debug('response end: ' + url + ' ==============================')
        resolve(res.data)
      } else {
        if (showToast) toast(res.data && res.data.message || '网络错误', 'error')
...
const setSession = (loginData) => {
  if (loginData) {
    let user = {
      ...
      'pregnentState': loginData.pregnentState,
      'expectBirthday': loginData.expectBirthday,
      'pregnancyWeek': countPregnancyWeek(loginData.expectBirthday),
      'babyAge': countBabyAge(loginData.babyBirthday),
      'isRegister': !(loginData.notregister === 'true')
    }
    db.SetUser(user)
    db.Set('token', loginData.usertoken)
    db.Set('orderNum', loginData.orderNum)
    db.Set('reportNum', loginData.reportNum)
...
const getSession = () => {
  return {
    user: db.GetUser(),
    token: db.Get('token'),
    orderNum: db.Get('orderNum'),
    reportNum: db.Get('reportNum')
  }
}
